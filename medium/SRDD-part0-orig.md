...
As a developer, you should still care about how something is written, but the insight is this: insist the AI writes methods like `sort-RedBlackTree()` rather than `sort()` so you maintain a high-level understanding of what's happening without needing to trace through every line.  This approach to small well named functions / methods is one of the principals in Uncle Bob's "Clean Code" - see [summary of this](https://gist.github.com/wojteklu/73c6914cc446146b8b533c0988cf8d29#functions-rules), [general video](https://youtu.be/7EmboKQH8lM) and [book](https://www.goodreads.com/book/show/3735293-clean-code).

## The Honeymoon Phase

Like any developer new to AI coding, I started with the "vibe coding" approach. I didn't think much about the context window limitations and happily churned out utilities for my environment. I rapidly produced fully-functional, fully-documented, and fully-tested scripts — solutions I'd previously dismissed as too much work to fit into spare time. With me, if I don't complete a project quickly, I never get back to it. I get bored or consumed by other projects. So the rapid development cycle was gold.
...
Got it — the goal isn’t to dump Part 2 wholesale, but to **fold the missing structure, taxonomy, and clarity from Part 2 into Part 1**, while keeping Part 1’s more mature narrative voice and flow.

Below is a **merged, cleaned version of Part 1**, with:

* the **five-approach landscape** distilled (without list fatigue),
* clearer positioning of SRDD relative to vibe / agentic / context / SDD,
* duplication removed,
* and Part 1’s story arc preserved.

I’ve kept this at *Medium-appropriate length* and left the deep workflow mechanics for Part 2 (or a follow-up).
...
I first encountered this while writing long-form essays — expecting the AI to maintain continuity across chapters, only to discover it quietly rewriting earlier material. Later, the same thing happened in code. As systems grew, context windows became constraints rather than conveniences.
